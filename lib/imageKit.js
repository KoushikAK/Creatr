...
export const buildTransformationUrl = (src, transformations = []) => {
  ...
  const transformParams = transformations
    .map((transform) => {
      // For standard transformations like resize, crop, and effects
      const params = [];
      if (transform.width) params.push(`w-${transform.width}`);
      if (transform.height) params.push(`h-${transform.height}`);
      if (transform.focus) params.push(`fo-${transform.focus}`);
      ...
      if (transform.overlayText) {
        const layerParams = [
          `l-text`,
          `i-${encodeURIComponent(transform.overlayText)}`,
          `tg-bold`, // You can keep bold or make it dynamic
        ];
        ...
        // This now correctly uses gravity for positioning instead of a hardcoded value
        if (transform.gravity) {
           layerParams.push(`g-${transform.gravity}`);
        }
        if (transform.overlayTextPadding) {
          layerParams.push(`pa-${transform.overlayTextPadding}`);
        }
        ...
        // This joins the standard params (if any) with the layer params
        const layerString = `l-layer,${layerParams.join(",")},l-end`;
        params.push(layerString);
      }
  ...
  // === CORRECTED URL INSERTION LOGIC ===
  try {
    const url = new URL(src);
    const pathParts = url.pathname.split("/").filter(p => p);
    
    // Ensure it's a valid-looking ImageKit path
    ...
    // Assemble the final, valid URL
    return `${endpoint}/${imageKitId}/tr:${transformParams}/${imagePath}`;
  } catch (error) {
